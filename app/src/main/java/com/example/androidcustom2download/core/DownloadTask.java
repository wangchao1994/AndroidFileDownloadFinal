...
import com.example.androidcustom2download.core.ConnectThread.ConnectListener;
import com.example.androidcustom2download.core.DownloadThread.DownloadListener;
import com.example.androidcustom2download.entity.DownloadEntry;
import com.example.androidcustom2download.entity.DownloadEntry.DownloadStatus;
import com.example.androidcustom2download.utils.FileUtils;
...
public class DownloadTask implements ConnectListener, DownloadListener{
	...
	public void start() {
		...
		if(entry.totalLength > 0){
			//本地数据库有历史记录
			LogUtils.d("DownloadTask===>start()#####no need to request content length!");
			startDownload();
		}else{
			//第一次下载
			entry.status = DownloadStatus.connecting;
			LogUtils.d("DownloadTask===>start()#####first start download" + "*****" + entry.toString());
	...
	private void startMultiThreadDownload(){
		...
		if(flag){
			...
			LogUtils.d("DownloadTask==>" + "startMultiThreadDownload#####" +entry.name + " is already done");
		}
	}
	
	private void startSingleThreadDownload(){
		entry.status = DownloadStatus.downloading;
		LogUtils.d(entry.toString());
		notifyUpdate(entry, DownloadService.NOTIFY_DOWNLOADING);
	...
	private void startDownload() {
		...
		if(FileUtils.getStorageAvailableSize() <= entry.totalLength){
			...
			LogUtils.d("DownloadTask==>" + "onConnectSuccess##### not enough storage size!!!");
			notifyUpdate(entry, DownloadService.NOTIFY_NOT_ENOUGH_SIZE);
			return;
		}
		
		if(entry.isSupportRange){
			LogUtils.d("DownloadTask()==>" + "startDownload#####start multi thread download!!!!");
			startMultiThreadDownload();
		}else{
			LogUtils.d("DownloadTask()==>" + "startDownload#####start single thread download!!!!");
	...
	public void onConnectSuccess(boolean isSupportRange, int totalLength) {
		...
		LogUtils.d("DownloadTask==>" + "onConnectSuccess#####" + entry.toString());
		startDownload();
	}
	...
	public void onConnectFaile(String message) {
		if (isPaused || isCancelled) {
            ...
            LogUtils.d("DownloadTask==>" + "onConnectFaile#####" + "isPaused or isCancelled is true*****" + entry.toString());
        } else {
            entry.status = DownloadEntry.DownloadStatus.error;
            notifyUpdate(entry, DownloadService.NOTIFY_ERROR);
            LogUtils.d("DownloadTask==>" + "onConnectFaile#####" + "error*****" + entry.toString());
        }
	}
	...
	private void checkAndRefreshSubThread(int index) {
		...
		for (int i = 0; i < downloadThreads.length; i++) {
			if(stamp - lastModifiedTime[i] > 2 * subThreadRefreshInterval){
				...
				if(startPos < endPos){
					LogUtils.d("DownloadTask==>onProgressChanged()###" + " restart sub-thread " + i);
		...
		if(stamp - lastModifiedTime[index] > subThreadRefreshInterval){
			...
			LogUtils.d("DownloadTask==>checkAndRefreshSubThread()##### index: " 
	...
	public synchronized void onDownloadCompleted(int index) {
		LogUtils.d("DownloadTask==>onDownloadCompleted():index==>" + index);
		...
		if(entry.totalLength > 0 && entry.currentLength != entry.totalLength){
			...
			entry.reset();
			notifyUpdate(entry, DownloadService.NOTIFY_ERROR);
			LogUtils.d("DownloadTask==>onDownloadCompleted()#####file is error, reset it!!!!!");
		}else{
			//文件下载完成，没有异常
			entry.status = DownloadStatus.done;
			entry.percent = 100;
            notifyUpdate(entry, DownloadService.NOTIFY_COMPLETED);
            LogUtils.d("DownloadTask==>onDownloadCompleted()#####file is ok!!!!!");
		}
	}

	@Override
	public synchronized void onDownloadError(int index, String message) {
		LogUtils.e("onDownloadError:" + message);
